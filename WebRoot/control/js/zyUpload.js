function checkPost(){
		if($("#title").val().length>15
			...
			show_message2("请仔细填写信息","error");
			return false;
		} 
}
function show_message2(text,type){
	var message = {
		text:text,
		type:(type)
	}
	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					...
					url              : "/assistant/UploadAction",  	// 上传文件的路径
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >= 2097152) {
						show_message("请选择2m以内的图片","error");	
					} else {
						// 在这里需要判断当前所有文件中
						if(file.type.indexOf("png") < 0 && file.type.indexOf("jpeg") < 0 )
						{
							show_message("只支持 jpg png 格式的图片","error");	
						}
						else{
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						$("#status_info").append("上传成功");
						
						$("#puc_url").val($("#puc_url").val()+ ";"+response);
						//$("#puc_url").append("<input>" + response);
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
